...
public class EeeLink {
	...
	state = EeeState.TRANSITION_TO_FAST;
	...
	if (DualModeEeeSimulator.max_delay_timer > 0 && queue_size == 1 && state != EeeState.ACTIVE) {
	    long max_delay_event_time = event.time + DualModeEeeSimulator.max_delay_timer - DualModeEeeSimulator.deep_to_active_t;
	    DualModeEeeSimulator.event_handler.addEvent(new StateTransitionEvent (max_delay_event_time, "handleStateTransitionEvent", EeeState.TRANSITION_TO_ACTIVE_FROM_DEEP));
	}
    }
	...
	if (queue_size > 0) {
	    ...
	    DualModeEeeSimulator.event_handler.addEvent(new StateTransitionEvent (event.time, "handleStateTransitionEvent", EeeState.TRANSITION_TO_FAST));
	}
    }
	...
	if (event.new_state == EeeState.ACTIVE) {
		...
		DualModeEeeSimulator.event_handler.addEvent(new StateTransitionEvent (event.time + DualModeEeeSimulator.fast_to_deep_timer, "handleStateTransitionEvent", EeeState.TRANSITION_TO_DEEP));
	    }
	    ...
	    DualModeEeeSimulator.event_handler.addEvent(new StateTransitionEvent (event.time + DualModeEeeSimulator.fast_to_deep_t, "handleStateTransitionEvent", EeeState.DEEP_SLEEP));
	} else if (event.new_state == EeeState.TRANSITION_TO_ACTIVE_FROM_FAST || event.new_state == EeeState.TRANSITION_TO_ACTIVE_FROM_DEEP) {
	    if (DualModeEeeSimulator.max_delay_timer > 0) {
		DualModeEeeSimulator.event_handler.removeStateTransitionEvent(EeeState.TRANSITION_TO_ACTIVE_FROM_DEEP);
	    }
